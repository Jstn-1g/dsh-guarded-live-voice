...
function hasExactDisclosure(value, expected) {
	return isRecord$2(value) && hasOnlyKeys(value, DISCLOSURE_KEYS) && DISCLOSURE_KEYS.every((key) => value[key] === expected[key]);
}
...
var VoiceSessionManager = class {
	...
	constructor(authority, consents, authorizeProvider, provider = "qwen") {
		this.authority = authority;
		this.consents = consents;
		this.authorizeProvider = authorizeProvider;
		this.provider = provider;
		if (provider !== "qwen" && provider !== "synthetic-demo") throw new TypeError("voice provider must be qwen or synthetic-demo");
	}
	...
	async acceptConsent(connectionId, challenge) {
		...
		try {
			...
			if (provider.provider !== this.provider) throw new GuardedVoiceError("invalid-state", "authorized provider does not match the disclosed provider");
...
//#endregion
//#region src/host/synthetic-demo-turn.ts
const SYNTHETIC_DEMO_PROVIDER = "synthetic-demo";
...
function buildDemoChime() {
	const sampleCount = OUTPUT_PCM_SAMPLE_RATE * CHIME_DURATION_MS / 1e3;
	const bytes = new Uint8Array(sampleCount * 2);
	const view = new DataView(bytes.buffer);
	for (let index = 0; index < sampleCount; index += 1) {
		const phase = index % CHIME_CYCLE_SAMPLES;
		const triangle = phase < CHIME_QUARTER_CYCLE_SAMPLES ? phase : phase < 36 ? 24 - phase : phase - CHIME_CYCLE_SAMPLES;
		const envelope = Math.min(index, 2399 - index, CHIME_FADE_SAMPLES);
		const sample = Math.trunc(triangle * CHIME_TRIANGLE_SCALE * envelope / CHIME_FADE_SAMPLES);
		view.setInt16(index * 2, sample, true);
	}
	if (bytes.byteLength === 0 || bytes.byteLength > 65536 || bytes.byteLength % 2 !== 0) throw new Error("synthetic demo chime exceeds the output boundary");
	return bytes;
}
...
*/
async function openSyntheticDemoTurn(options) {
	if (options.signal.aborted) throw new GuardedVoiceError("invalid-state", "synthetic demo turn was cancelled");
	...
	const session = {
		...
		closed,
		appendPcm16(chunk) {
			if (state !== "input") throw new GuardedVoiceError("invalid-state", "synthetic demo turn is not accepting audio");
			if (chunk.byteLength === 0 || chunk.byteLength > 32768 || chunk.byteLength % 2 !== 0) throw new GuardedVoiceError("invalid-message", "synthetic demo PCM16 input chunk is invalid");
			if (inputBytes + chunk.byteLength > 96e4) throw new GuardedVoiceError("invalid-message", "synthetic demo PCM16 input exceeds the turn limit");
			inputBytes += chunk.byteLength;
		},
		commit() {
			if (state !== "input") throw new GuardedVoiceError("invalid-state", "synthetic demo turn can be committed only once");
			if (inputBytes === 0) throw new GuardedVoiceError("invalid-state", "synthetic demo turn has no audio to commit");
			state = "response";
			defer(() => {
				if (!emit({
					type: "transcript",
					role: "user",
					text: "Synthetic demo request: place this sample transcript in the DSH draft.",
					final: true
				})) return;
				if (!emit({
					type: "transcript",
					role: "assistant",
					text: "Synthetic demo response: the local consent-bound turn completed without contacting an external provider.",
					final: true
				})) return;
				if (!emit({
					type: "audio",
					pcm24: new Uint8Array(DEMO_CHIME)
				})) return;
...
const Config = z.object({
	provider: z.union(["qwen", "synthetic-demo"]).default("qwen"),
...
function apply(ctx, input) {
	...
	if (config.provider === "qwen") {
		...
		openProvider = async (_binding, authorization, signal) => {
			signal.throwIfAborted();
			if (authorization.provider !== "synthetic-demo" || authorization.model !== "dsh-live-voice-synthetic-demo-v1") throw new GuardedVoiceError("provider-unconfigured", "provider authorization does not match synthetic demo configuration");
			return openSyntheticDemoTurn({ signal });
		};
	}
	const manager = new VoiceSessionManager(new AuthorityGuard({ get: (sessionId) => ctx.sessions.get(SessionId(sessionId)) }, { list: () => ctx.workspaceRegistry.list() }), new ConsentChallenges({ ttlMs: config.consentTtlMs }), authorizeProvider, config.provider);
	const gateway = new GuardedVoiceGateway({
		manager,
		turns: new ManualTurnCoordinator(manager, openProvider),
		trustedHosts,
...
//#endregion
export { AuthorityGuard, CLIENT_BOOT_GLOBAL, CLIENT_BOOT_VERSION, Config, ConsentChallenges, DEFAULT_QWEN_INPUT_TIMEOUT_MS, DEFAULT_QWEN_REALTIME_MODEL, DEFAULT_QWEN_RESPONSE_TIMEOUT_MS, GuardedVoiceError, MAX_CONTROL_BYTES, MAX_PROPOSAL_INSTRUCTION_LENGTH, MAX_PROPOSAL_TITLE_LENGTH, MAX_QWEN_BUFFERED_BYTES, MAX_QWEN_INPUT_CHUNK_BYTES, MAX_QWEN_INPUT_TURN_BYTES, MAX_QWEN_OUTPUT_CHUNK_BYTES, MAX_QWEN_OUTPUT_TURN_BYTES, MAX_QWEN_PHASE_TIMEOUT_MS, MAX_QWEN_PROVIDER_CONTROL_BYTES, MAX_QWEN_REALTIME_EVENT_BYTES, MAX_QWEN_TRANSCRIPT_LENGTH, ManualTurnCoordinator, PROPOSAL_TOOL_NAME, QWEN_REALTIME_MODELS, QwenHandshake, SYNTHETIC_DEMO_ASSISTANT_TRANSCRIPT, SYNTHETIC_DEMO_MODEL, SYNTHETIC_DEMO_PROVIDER, SYNTHETIC_DEMO_USER_TRANSCRIPT, VoiceSessionManager, WIRE_VERSION, apply, assertTrustedHosts, assessUpgradeRequest, buildQwenRealtimeEndpoint, encodeServerControl, guardedVoiceClientBootInjection, inject, isQwenRealtimeModel, openQwenManualTurn, openSyntheticDemoTurn, parseClientControl, parseGuardedProposal, parseGuardedVoiceClientBoot, parseServerControl };
